...
const THREAD0_PRIORITY: u8 = 1;
const THREAD1_PRIORITY: u8 = 2;
const THREAD2_PRIORITY: u8 = 3;
...
pub fn make_thread() {
    ...
    assert_eq!(
        thread0.base_priority(),
        Priority::thread_priority(THREAD0_PRIORITY)
    );
    ...
    assert_eq!(
        thread1.base_priority(),
        Priority::thread_priority(THREAD1_PRIORITY)
    );
    ...
    thread1.start(move || {
        ...
        assert_eq!(
            thread2.base_priority(),
            Priority::thread_priority(THREAD2_PRIORITY)
        );
