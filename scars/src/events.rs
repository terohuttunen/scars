    ...
    pub fn try_wait(&mut self) -> Result<Events, TryWaitError> {
        match Scheduler::current_execution_context() {
            ...
            ExecutionContext::Interrupt(current_interrupt) => {
                ...
                let all_pending = current_interrupt.pending_events.load(Ordering::SeqCst);
                
                // Check if we should resume based on wait conditions
                if self.should_resume(all_pending) {
                    // Process events based on options
                    let (received_events, events_to_clear) = 
                        self.process_and_store_pending_events(all_pending);
                    
                    // Atomically clear consumed events
                    let remaining = all_pending & !events_to_clear;
                    current_interrupt.pending_events.store(remaining, Ordering::SeqCst);
                    
                    // Return consumed events
                    Ok(received_events)
                } else {
                    // Would block - return available events that match our wait mask
                    let matched_available = all_pending & self.events();
                    Err(TryWaitError::WouldBlock(matched_available))
                }
            }
        }
    }
...
/// Builder for constructing WaitEvents with compile-time validation using typestates.
///
