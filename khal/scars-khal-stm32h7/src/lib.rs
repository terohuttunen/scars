...
pub unsafe extern "C" fn tim2() {
    core::arch::naked_asm!(
        ...
        "movt   r1, 0x4000",
        "ldr    r3, [r1]",
        // Fast-path return runs before any bl, so lr still holds EXC_RETURN.
        "cmp    r2, r3",
        "it     lt",
        "bxlt   lr",
        // Expired path. Inline the slot load via `sym` resolution to
        // the macro-emitted CURRENT_THREAD_CONTEXT. r4 caches
        // old Context* across the handler (callee-saved); r5/r6 pad
        // the push to a 4-register / 16-byte / 8-byte-aligned frame.
        "push   {{r4, r5, r6, lr}}",
        "ldr    r4, ={slot}",
        "ldr    r4, [r4]",
